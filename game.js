...
// ================================================================
const TUT_MAP_W = 36, TUT_MAP_H = 9;
...
// 습격 직후 화이트플래시와 동시에 카메라를 '뚝' 끊어 옮기는 전용 공간 — 메인 통로와 연결되지 않은 독립된 영역
// (정밀소나로 좀비떼에 둘러싸이는 연출을 위해 1칸 폭 통로보다 넓은 공간이 필요해서 따로 마련함)
// 벽으로 딱 맞게 가두면 화면(뷰포트)보다 좁아서 가장자리가 비어 보이므로, 뷰포트보다 넉넉하게 넓은 개방형 바닥으로 깔아둠
const TUT_VOID_ROOM_X0 = 15, TUT_VOID_ROOM_X1 = 35; // x 15~35 (21칸 폭)
const TUT_VOID_ROOM_Y0 = 0,  TUT_VOID_ROOM_Y1 = 8;  // y 0~8 (맵 전체 높이)
const TUT_VOID_ROOM_TX = 25, TUT_VOID_ROOM_TY = 4;  // 영역 중앙 — 텔레포트 목적지
...
function generateTutorialMap() {
  ...
  // 습격 직후 카메라컷용 독립 공간 — 메인 통로와 사이에 벽 1칸 이상 띄워서 플레이어가 평소엔 못 들어가게 함
  for (let y = TUT_VOID_ROOM_Y0; y <= TUT_VOID_ROOM_Y1; y++) {
    for (let x = TUT_VOID_ROOM_X0; x <= TUT_VOID_ROOM_X1; x++) {
...
// ── 습격 직후 화이트플래시와 동시에 호출 — 카메라를 부드러운 전환 없이 '뚝' 끊어서 넓은 방으로 옮김 ──
function teleportTutorialToVoidRoom() {
  const ts = CONFIG.map.tileSize;
  player.tx = TUT_VOID_ROOM_TX;
  player.ty = TUT_VOID_ROOM_TY;
  player.px = player.targetX = TUT_VOID_ROOM_TX * ts;
  player.py = player.targetY = TUT_VOID_ROOM_TY * ts;
  player.moving = false;
  // 카메라도 같은 프레임에 즉시 스냅 — 스무딩(보간) 없이 한 번에 전환해야 '뚝' 끊기는 느낌이 남
  camX = player.px + ts / 2 - viewW() / 2;
  camY = player.py + ts / 2 - viewH() / 2;
  revealAround(player.tx, player.ty, 9); // 넓어진 영역을 즉시 밝힘 (좀비 노출 때와 동일 반경)
...
function onTutorialAmbushResolved() {
  ...
  teleportTutorialToVoidRoom(); // 화이트플래시와 동시에 카메라를 뚝 끊어 넓은 방으로 이동 — 정밀소나로 둘러싸이는 연출 공간 확보
  triggerOriginFlash(); // 기존 화이트플래시 100% 재사용 — 순간이동의 시각적 핑계 역할도 겸함
...
// ── 정밀소나용 좀비 스폰 — 개방형 공간에서 가까운 칸을 우선 채워 플레이어를 사방으로 에워싸고,
//    겹쳐서(픽셀 오프셋) 배치해 바글바글한 느낌을 줌. AI 동결 상태로 대기 ──
function spawnTutorialPreciseZombies() {
  const ts = CONFIG.map.tileSize;
  TUT_FREEZE_ZOMBIES = true; // 다가오지 않도록 AI 동결

  // 주변 8방향 × 거리 1~8칸으로 바닥 타일 후보 수집 — 개방형 공간이라 모든 방향이 살아남음
  const candidates = [];
  ...
  if (candidates.length === 0) candidates.push({ tx: player.tx, ty: player.ty, dist: 1 });

  // 가까운 칸(플레이어 바로 주변)일수록 풀에 더 많이 넣어서 사방을 우선 에워싸듯 채우고,
  // 남는 마리는 안쪽부터 바깥으로 확장
  const pool = [];
  for (const c of candidates) {
    const weight = c.dist <= 2 ? 4 : c.dist <= 4 ? 2 : 1;
    for (let w = 0; w < weight; w++) pool.push(c);
  }
  // 셔플 — 랜덤 추첨 시 같은 칸에 우연히 몰려서 '깔끔한 짝'처럼 보이던 문제 방지(고르게 순회)
  ...
  const count = 28; // 개방형 공간이 넓어진 만큼 좀비 수도 늘려서 밀도를 유지
  for (let i = 0; i < count; i++) {
    const c = pool[i % pool.length];
    const z = makeZombieObj(c.tx, c.ty, ts, 'BASIC', 'CREATURE');
    z.px += (Math.random() - 0.5) * ts * 0.85; // 지터 범위 확대 — '붙어있는 짝'이 아니라 흩어진 무리로 보이게
    z.py += (Math.random() - 0.5) * ts * 0.85;
    z.state = 'WANDER';
    zombies.push(z);
  }
}
...
function onTutorialPreciseFired() {
  ...
  revealAround(player.tx, player.ty, 9); // 좀비들이 보이도록 시야 확보 (확장된 영역 전체)
  // 비네팅 해제를 핑 확산 진행률(sonar.pulseR/pulseMaxR)에 직접 연동
  // — 정밀소나가 퍼지는 것과 정확히 같은 속도로 시야가 풀림
  TUT_VIGNETTE.active = true;
  TUT_VIGNETTE.mode = 'precise_release';

  // 소나가 퍼지는 동안 좀비떼가 드러나는 느낌을 강화 — 식별 사운드(그르렁) 2~4회를 살짝 시차를 두고 재생
  const growlCount = 2 + Math.floor(Math.random() * 3); // 2~4회
  for (let i = 0; i < growlCount; i++) {
    setTimeout(() => {
