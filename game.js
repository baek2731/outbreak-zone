...
function spawnTutorialPreciseZombies() {
  ...
  // 주변 8방향 × 거리 1~6칸으로 바닥 타일 후보 수집 — 통로 폭이 1칸이라 실제로는 3방향(좌/우/아래) 정도만 살아남지만,
  ...
  // 가까운 칸(플레이어 바로 주변)일수록 풀에 더 많이 넣어서 — 통로가 허락하는 모든 방향에서
  // 플레이어를 우선 에워싸듯 채우고, 남는 마리는 안쪽부터 깊이로 확장
  const pool = [];
